...
export default function HelloWorld(props: IHelloWorldProps){
    ...
    let loadInitialData = async ()=>{
      let response = await props.spContext.spHttpClient.get(props.spContext.pageContext.web.absoluteUrl+ "/_api/web/allProperties", SPHttpClient.configurations.v1);
    ...
    let loadWelcomePage = async ()=>{
      let response = await props.spContext.spHttpClient.get(props.spContext.pageContext.web.absoluteUrl+ "/_api/web?$select=WelcomePage", SPHttpClient.configurations.v1);
        ...
        <div className={ styles.container }>
          <div className={ styles.row }>
            <div className={ styles.column }>
              ...
              {webProperties ? <div data-testid="web-property-div" >{webProperties.ThemePrimary}</div> : <Spinner data-testid="loading-spinner" />}
            </div>
          </div>
          <div className={ styles.row }>
            <div className={ styles.column }>
              <span className={ styles.title }>Welcome page is</span>
              {welcomePage ? <div data-testid="welcome-page-div" >{welcomePage}</div> : <PrimaryButton 
